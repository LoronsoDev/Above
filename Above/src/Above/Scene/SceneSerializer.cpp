...
//This file is mostly taken from "Hazel" on GitHub.
...
namespace Above
{
	...
	bool SceneSerializer::DeserializeText(const std::string& filepath)
	{
		...
		AB_CORE_TRACE("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				AB_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
